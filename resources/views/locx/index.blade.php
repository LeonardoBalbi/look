        ...
        @elseif ($page === 'whatsapp')
            <div class="panel"><h2>WhatsApp Business API</h2><p>Configure a integração oficial da Meta. Use o modo <strong>demo</strong> para testar sem token real. Versão da Graph API: <strong>{{ $graphVersion }}</strong>.</p>
                <form method="post" action="{{ route('locx.whatsapp.salvar') }}" class="form-grid">@csrf
                    ...
                    <label>Phone Number ID<input name="phone_number_id" value="{{ $whatsappConfig->phone_number_id }}"></label><label class="span-3">Access Token<input type="password" name="access_token" value="{{ $whatsappConfig->access_token }}"></label><label>Verify Token<input name="verify_token" value="{{ $whatsappConfig->verify_token ?: 'locx_webhook_token' }}"></label>
                    <label>Template cobrança (1 dia)<input name="template_cobranca" value="{{ $whatsappConfig->template_cobranca }}"></label>
                    <label>Template lembrete (3 dias antes)<input name="template_lembrete" value="{{ $whatsappConfig->template_lembrete }}"></label>
                    <label>Template vencimento + PIX<input name="template_vencimento" value="{{ $whatsappConfig->template_vencimento }}"></label>
                    <label>Template pagamento confirmado<input name="template_pagamento" value="{{ $whatsappConfig->template_pagamento }}"></label>
                    <label>Template aviso ao gerente<input name="template_gerente" value="{{ $whatsappConfig->template_gerente }}"></label>
                    <label>WhatsApp do gerente<input name="gerente_whatsapp" value="{{ $whatsappConfig->gerente_whatsapp }}" placeholder="Ex.: 5521999999999"></label>
                    <label>Template bloqueio<input name="template_bloqueio" value="{{ $whatsappConfig->template_bloqueio }}"></label>
                    <div class="span-3"><button type="submit">Salvar Configuração</button></div>
                </form>
                <p><strong>n8n:</strong> {{ config('n8n.enabled') ? 'automações ativas' : 'automações desativadas para segurança' }}.</p>
            </div>
            <div class="grid side"><div class="panel"><h2>Testar conexão</h2><form method="post" action="{{ route('locx.whatsapp.testar') }}">@csrf<button class="btn success">Validar credenciais</button></form><p><strong>URL do Webhook:</strong><br><code>{{ route('locx.webhook-whatsapp') }}</code></p><p><a class="btn secondary" href="{{ \App\Support\Locx::asset('docs/manual_whatsapp.html') }}" target="_blank">Abrir manual WhatsApp</a></p></div>
                <div class="panel"><h2>Últimos envios</h2><div class="table-wrap"><table><tr><th>Data</th><th>Cliente</th><th>Telefone</th><th>Tipo</th><th>Status</th><th>HTTP</th></tr>@foreach($whatsappLogs as $log)<tr><td>{{ $log->criado_em?->format('d/m/Y H:i') }}</td><td>{{ $log->cliente?->nome ?? '-' }}</td><td>{{ $log->telefone }}</td><td>{{ $log->tipo }}</td><td>{!! \App\Support\Locx::status($log->status) !!}</td><td>{{ $log->http_code ?? '-' }}</td></tr>@endforeach</table></div></div>
            </div>
