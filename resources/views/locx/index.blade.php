...
Caso já tenha pago, desconsidere esta mensagem.") }}</textarea><small>Variáveis: {cliente}, {cobranca_id}, {vencimento}, {valor}, {saldo}, {dias_atraso}, {placa}, {pix}, {link_portal}.</small></label>
        ...
        @elseif ($page === 'lojas')
            <div class="grid side">
                <div class="panel"><h2>{{ $lojaEdit ? 'Editar loja' : 'Nova loja' }}</h2>@if(!$podeCriarLoja && !$lojaEdit)<div class="notice warn">Seu perfil pode visualizar lojas, mas nao criar novas unidades.</div>@endif
                    <form method="post" action="{{ route('locx.lojas.salvar') }}" class="form-grid">@csrf
                    ...
                    @if($lojaEdit)<p><a class="btn secondary" href="{{ route('locx.index', ['page' => 'bancos', 'loja_id' => $lojaEdit->id]) }}">Configurar bancos desta loja</a></p>@endif
                </div>
                <div class="panel"><h2>Controle por Loja</h2><div class="table-wrap"><table><tr><th>Loja</th><th>Status</th><th>Motos</th><th>Alugadas</th><th>Disponiveis</th><th>Recebido</th><th>Em atraso</th><th>Acoes</th></tr>
                    @foreach($resumoLojas as $item)<tr><td>{{ $item['loja']->nome }}<br><small>{{ $item['loja']->cidade ?: '-' }}</small></td><td>{!! \App\Support\Locx::status($item['loja']->status) !!}</td><td>{{ $item['motos'] }}</td><td>{{ $item['alugadas'] }}</td><td>{{ $item['disponiveis'] }}</td><td>{{ \App\Support\Locx::moeda($item['recebido']) }}</td><td>{{ \App\Support\Locx::moeda($item['atraso']) }}</td><td><a class="btn secondary" href="{{ route('locx.index', ['page' => 'lojas', 'edit' => $item['loja']->id]) }}">Editar</a> <a class="btn secondary" href="{{ route('locx.index', ['page' => 'bancos', 'loja_id' => $item['loja']->id]) }}">Bancos</a></td></tr>@endforeach
                </table></div></div>
            </div>
        ...
        @elseif ($page === 'bancos')
            ...
            @if ($bancoSelecionado === 'pagbank')
                <div class="grid side">
                    <div class="panel"><h2>Configuração PagBank / PIX</h2><form method="post" action="{{ route('locx.pagbank.salvar') }}" class="form-grid">@csrf<input type="hidden" name="page" value="bancos"><input type="hidden" name="loja_id" value="{{ $lojaBancoId ?? '' }}">
            ...
            @elseif ($bancoSelecionado === 'asaas')
                <div class="grid side">
                    <div class="panel"><h2>Configuração Asaas / PIX</h2><form method="post" action="{{ route('locx.asaas.salvar') }}" class="form-grid">@csrf<input type="hidden" name="page" value="bancos"><input type="hidden" name="loja_id" value="{{ $lojaBancoId ?? '' }}">
                    ...
                    <div class="panel"><h2>Status da integração</h2><p><strong>Webhook:</strong><br><code>{{ $asaasConfig->webhook_url ?: route('locx.webhook-asaas') }}</code></p><p><strong>Ambiente:</strong> {{ $asaasConfig->ambiente }} · <strong>Modo:</strong> {{ $asaasConfig->modo === 'api' ? 'api oficial' : 'demo' }}</p><p><strong>Gateway PIX principal:</strong> {{ ['asaas' => 'Asaas', 'sicoob' => 'Sicoob'][$pixGatewayConfig->gateway] ?? 'PagBank' }}</p><form method="post" action="{{ route('locx.gateway-pix.salvar') }}" class="form-grid">@csrf<input type="hidden" name="page" value="bancos"><input type="hidden" name="loja_id" value="{{ $lojaBancoId ?? '' }}"><label>Usar para gerar PIX<select name="gateway"><option value="pagbank" @selected($pixGatewayConfig->gateway==='pagbank')>PagBank</option><option value="asaas" @selected($pixGatewayConfig->gateway==='asaas')>Asaas</option><option value="sicoob" @selected($pixGatewayConfig->gateway==='sicoob')>Sicoob</option></select></label><div><button class="btn secondary">Atualizar gateway</button></div></form><p>Cadastre a API Key, configure o webhook no painel Asaas e gere uma cobrança de teste.</p><p><a class="btn secondary" href="{{ \App\Support\Locx::asset('docs/manual_asaas.html') }}" target="_blank">Abrir manual Asaas</a></p></div>
                </div>
            @elseif ($bancoSelecionado === 'sicoob')
                <div class="grid side">
                    <div class="panel"><h2>Configuração Sicoob / PIX</h2><form method="post" action="{{ route('locx.sicoob.salvar') }}" class="form-grid">@csrf<input type="hidden" name="page" value="bancos"><input type="hidden" name="loja_id" value="{{ $lojaBancoId ?? '' }}">
            ...
            @else
                <div class="grid side">
                    <div class="panel"><h2>Configuracao Itau / PIX</h2><form method="post" action="{{ route('locx.itau.salvar') }}" class="form-grid">@csrf<input type="hidden" name="page" value="bancos"><input type="hidden" name="loja_id" value="{{ $lojaBancoId ?? '' }}">
