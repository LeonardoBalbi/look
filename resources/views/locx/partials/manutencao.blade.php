<div class="cards">
    ...
    <div class="metric"><span>Custo previsto</span><strong>{{ \App\Support\Locx::moeda($manutencaoResumo['custoPrevisto']) }}</strong></div>
    <div class="metric ok"><span>Custo final no mês</span><strong>{{ \App\Support\Locx::moeda($manutencaoResumo['custoFinalMes']) }}</strong></div>
</div>

<div class="grid side">
    <div class="panel">
        <h2>{{ $ordemEdit ? 'Editar' : 'Nova' }} Ordem de Serviço</h2>
        <form method="post" action="{{ route('locx.manutencao.ordens.salvar') }}" class="form-grid">
            @csrf
