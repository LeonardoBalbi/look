<div class="cards">
    ...
    <div class="metric danger"><span>Valor aberto</span><strong>{{ \App\Support\RentalSupport::moeda($multasResumo['valorAberto']) }}</strong></div>
...
<div class="grid side">
    <div class="panel">
        ...
        <form method="post" action="{{ route('rental.multas.salvar') }}" class="form-grid">
            @csrf
            <input type="hidden" name="id" value="{{ $multaEdit?->id }}">
            @if($singleStore)<input type="hidden" name="loja_id" value="{{ $multaEdit?->loja_id ?: $lojaUnica?->id }}">@else<label>Loja<select name="loja_id"><option value="">Selecione</option>@foreach($lojas as $loja)<option value="{{ $loja->id }}" @selected(old('loja_id',$multaEdit?->loja_id)==$loja->id)>{{ $loja->nome }}</option>@endforeach</select></label>@endif
    ...
    <div class="panel">
        ...
        <div class="table-wrap"><table><tr><th>ID</th><th>Moto</th><th>Cliente</th><th>Valor</th><th>Vencimento</th><th>Status</th><th>Ações</th></tr>
            @foreach($multasTransito as $multa)
                <tr><td>#{{ $multa->id }}</td><td>{{ $multa->motocicleta?->placa ?? '-' }}</td><td>{{ $multa->cliente?->nome ?? '-' }}</td><td>{{ \App\Support\RentalSupport::moeda($multa->valor) }}</td><td>{{ $multa->vencimento?->format('d/m/Y') ?? '-' }}</td><td>{!! \App\Support\RentalSupport::status($multa->status) !!}</td><td><a class="btn secondary" href="{{ route('rental.index',['page'=>'multas','edit'=>$multa->id]) }}">Editar</a></td></tr>
            @endforeach
        </table></div>
    </div>
</div>
