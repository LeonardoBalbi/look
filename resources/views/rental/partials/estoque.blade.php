    ...
    <div class="panel">
        ...
        <div class="table-wrap"><table><tr><th>Data</th><th>Produto</th><th>Tipo</th><th>Qtd.</th><th>Origem</th></tr>
            @foreach($movimentosEstoque as $movimento)
                <tr><td>{{ $movimento->movimentado_em?->format('d/m/Y H:i') }}</td><td>{{ $movimento->produto?->nome }}</td><td>{!! \App\Support\RentalSupport::status($movimento->tipo) !!}</td><td>{{ number_format((float) $movimento->quantidade, 2, ',', '.') }}</td><td>{{ $movimento->origem ?: '-' }}</td></tr>
            @endforeach
        </table></div>
    </div>
</div>
