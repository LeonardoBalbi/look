    ...
    <section class="crm-service-workspace">
        <aside class="crm-conversation-sidebar panel">
            ...
            <div class="crm-conversation-list"
                 ...
                 data-sync-url="{{ route('rental.crm.portal-atendimentos.inbox-sync') }}"
                ...
                @forelse($crmPortalInbox as $atendimento)
                    ...
                    @endphp
                    <a href="{{ route('rental.index', ['page' => 'crm', 'atendimento' => $atendimento->id]) }}#crmAtendimentoAtual"
        ...
        <main class="crm-chat-panel panel" id="crmAtendimentoAtual" tabindex="-1">
            @if($atendimentoAtual && $clienteAtual)
                <article class="crm-active-conversation {{ $statusClasses[$atendimentoAtual->status] ?? '' }}"
                         ...
                         data-sync-url="{{ route('rental.crm.portal-atendimentos.sync', $atendimentoAtual) }}"
                         data-read-url="{{ route('rental.crm.portal-atendimentos.lido', $atendimentoAtual) }}"
                         data-action-url="{{ route('rental.crm.portal-atendimentos.acao', $atendimentoAtual) }}">
                            ...
                            @php
                                ...
                                $nomeRemetente = $tipoRemetente === 'cliente'
                                    ...
                                    : ($tipoRemetente === 'humano'
                                        ? 'Atendente · '.($mensagem->remetente_nome ?: 'Equipe '.$branding['store_name'])
                    ...
                    @if($atendimentoAtual->status !== 'fechado')
                        <section class="crm-composer">
                            ...
                            <form method="post" action="{{ route('rental.crm.portal-atendimentos.responder', $atendimentoAtual) }}" class="crm-chat-reply" data-crm-chat-reply>
                                @csrf
        ...
        <aside class="crm-customer-context panel">
            @if($clienteAtual)
                ...
                <div class="crm-contact-actions">
                    ...
                    <a href="{{ route('rental.index', ['page' => 'clientes', 'edit' => $clienteAtual->id]) }}">Cadastro</a>
                </div>

                <details class="crm-context-details crm-schedule-details" open>
                    <summary>Criar tarefa de acompanhamento</summary>
                    <form method="post" action="{{ route('rental.crm.tarefas.salvar') }}" class="crm-stacked-form">
                        @csrf
                ...
                <section class="crm-context-section crm-attendance-history-section">
                    ...
                    <div class="crm-attendance-history">
                        @forelse($crmAtendimentosCliente as $historico)
                            <a href="{{ route('rental.index', ['page' => 'crm', 'atendimento' => $historico->id]) }}#crmAtendimentoAtual"
                ...
                <section class="crm-context-section">
                    ...
                    <div class="crm-financial-snapshot">
                        <article><span>Saldo aberto</span><strong>{{ \App\Support\RentalSupport::moeda($crmClienteResumo['saldo_aberto'] ?? 0) }}</strong></article>
                ...
                <section class="crm-context-section">
                    ...
                    <form method="post" action="{{ route('rental.crm.cliente', $clienteAtual) }}" class="crm-compact-form">
                        @csrf
                ...
                <section class="crm-note-card">
                    ...
                    <form method="post" action="{{ route('rental.crm.notas.salvar') }}" class="crm-note-form">
                        @csrf
                        <input type="hidden" name="cliente_id" value="{{ $clienteAtual->id }}">
                        <label>Tipo
                        <select name="tipo">
            ...
            <div class="crm-task-list modern">
                @forelse($crmTarefasAbertas->take(8) as $tarefa)
                    <article class="{{ $tarefa->prazo_em && $tarefa->prazo_em->isPast() ? 'is-overdue' : '' }}">
                        ...
                        <form method="post" action="{{ route('rental.crm.tarefas.concluir', $tarefa) }}">@csrf<button type="submit">Concluir</button></form>
                    </article>
                @empty
            ...
            <table class="crm-client-table">
                ...
                @forelse($crmClientes as $linha)
                    ...
                    <tr data-crm-client="{{ \Illuminate\Support\Str::lower($clienteLinha->nome.' '.$clienteLinha->whatsapp.' '.$clienteLinha->email) }}">
                        ...
                        <td><strong>{{ \App\Support\RentalSupport::moeda($linha['saldo_aberto']) }}</strong></td>
                        ...
                        <td><a class="btn secondary" href="{{ route('rental.index', ['page' => 'crm', 'cliente' => $clienteLinha->id]) }}">Abrir CRM</a></td>
                    </tr>
                @empty
