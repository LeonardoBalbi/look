...
class RentalSupport
{
    ...
    {
        return trim((string) config('branding.product_name', 'Gestor de Locações')) ?: 'Gestor de Locações';
    }
    ...
    public static function integrationPrefix(): string
    {
        $prefix = preg_replace('/[^A-Z0-9]/', '', mb_strtoupper((string) config('branding.integration_prefix', 'RENTAL')));

        return substr($prefix ?: 'RENTAL', 0, 12);
    }

    public static function billingReference(int $cobrancaId): string
    {
        return self::integrationPrefix().'-COBRANCA-'.$cobrancaId;
    }

    public static function billingReferenceId(?string $reference): ?int
    {
        $prefixes = array_unique([self::integrationPrefix(), 'LOCX']);
        $pattern = '/(?:'.implode('|', array_map(fn (string $prefix): string => preg_quote($prefix, '/'), $prefixes)).')-COBRANCA-(\d+)/i';

        return preg_match($pattern, (string) $reference, $match) ? (int) $match[1] : null;
    }

    public static function webhookToken(string $gateway): string
    {
        $configured = trim((string) config('services.'.$gateway.'.webhook_token'));
        if ($configured !== '') {
            return $configured;
        }

        return substr(hash_hmac('sha256', 'webhook:'.$gateway, (string) config('app.key')), 0, 48);
    }
    ...
    public const MODULOS = [
        ...
        'bancos' => 'Integrações de pagamento',
        'pagbank' => 'PagBank',
        'asaas' => 'Asaas',
        'sicoob' => 'Sicoob',
        'itau' => 'Itau',
    ...
    {
        $relative = 'application/'.ltrim($path, '/');
    ...
    public static function status(?string $status): HtmlString
    {
        ...
        $classe = [
            ...
            'alugada' => 'info', 'retirada' => 'info', 'confirmada' => 'info', 'enviado' => 'info', 'conciliado' => 'ok', 'transferida' => 'info', 'demo' => 'info', 'local' => 'info',
            'aberta' => 'warn', 'solicitada' => 'warn', 'em_analise' => 'warn', 'parcial' => 'warn', 'pendente' => 'warn', 'agendada' => 'warn', 'processando' => 'warn', 'em_andamento' => 'warn',
    ...
    public static function icon(string $modulo): HtmlString
    {
        $paths = [
            ...
            'itau' => '<rect x="4" y="5" width="16" height="14" rx="2"/><path d="M8 9h8"/><path d="M8 13h8"/><path d="M8 17h5"/>',
