...
class RentalController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $page = array_key_exists($request->string('page')->toString(), RentalSupport::MODULOS)
    ...
    private function lookModulo(string $page, User $user): array
    {
        ...
        $catalogo = [
            'reservas' => [
                ...
                'valor_label' => 'Valor estimado',
                'data_label' => 'Retirada prevista',
                'detalhes_placeholder' => 'Informe período, horário, caução, documentos pendentes e observações para a retirada.',
            ...
            'contas' => [
                ...
                'indicadores' => [
                    ['label' => 'Registros', 'valor' => (clone $registrosBase)->count(), 'tipo' => ''],
                    ['label' => 'A receber', 'valor' => RentalSupport::moeda($abertoFinanceiro), 'tipo' => 'warn'],
                    ['label' => 'Recebido no mes', 'valor' => RentalSupport::moeda($recebidoMes), 'tipo' => 'ok'],
                    ['label' => 'Atraso', 'valor' => RentalSupport::moeda($atrasoFinanceiro), 'tipo' => 'danger'],
                ],
                'rotinas' => [
        ...
        return [
            ...
            'lookRelacionados' => collect($catalogo[$page]['atalhos'])
                ->filter(fn (string $modulo) => array_key_exists($modulo, RentalSupport::MODULOS) && $user->pode($modulo))
                ->values()
                ->all(),
        ];
    }

    private function reservas(Request $request, User $user): array
    {
        $reservasBase = $this->scope(Reserva::query(), $user);
        $reservaEdit = $request->integer('edit')
            ? (clone $reservasBase)->findOrFail($request->integer('edit'))
            : null;

        return [
            'reservaEdit' => $reservaEdit,
            'reservas' => (clone $reservasBase)
                ->with(['loja', 'cliente', 'motocicleta', 'criadoPor'])
                ->orderBy('retirada_em')
                ->limit(160)
                ->get(),
            'reservasResumo' => [
                'preparacao' => (clone $reservasBase)->whereIn('status', ['solicitada', 'em_analise', 'confirmada'])->count(),
                'retiradasHoje' => (clone $reservasBase)->whereDate('retirada_em', today())->whereNotIn('status', ['concluida', 'cancelada'])->count(),
                'devolucoesHoje' => (clone $reservasBase)->whereDate('devolucao_em', today())->whereNotIn('status', ['concluida', 'cancelada'])->count(),
                'disponiveis' => $this->scope(Motocicleta::query(), $user)->where('status_operacional', 'disponivel')->count(),
            ],
            'clientesReserva' => $this->scope(Cliente::query(), $user)->orderBy('nome')->get(),
            'motosReserva' => $this->scope(Motocicleta::query(), $user)
                ->where(function (Builder $query) use ($reservaEdit): void {
                    $query->where('status_operacional', 'disponivel');
                    if ($reservaEdit?->motocicleta_id) {
                        $query->orWhereKey($reservaEdit->motocicleta_id);
                    }
                })
                ->orderBy('modelo')
                ->get(),
            'lojasReserva' => $this->lojasPermitidasParaFormulario($user),
            'reservasLegadas' => $this->scope(LookModuloRegistro::query(), $user)
    ...
    public function salvarReserva(Request $request): RedirectResponse
    {
        $reserva = $request->integer('id')
            ? $this->scope(Reserva::query(), $request->user())->findOrFail($request->integer('id'))
            : new Reserva;
        $this->autorizar($request->user(), 'reservas', $reserva->exists ? 'editar' : 'criar');
        ...
        $dados = $request->validate([
            ...
            'retirada_em' => ['required', 'date'],
            'devolucao_em' => ['required', 'date', 'after:retirada_em'],
            'valor_estimado' => ['nullable', 'numeric', 'min:0'],
            'caucao' => ['nullable', 'numeric', 'min:0'],
            'origem' => ['required', Rule::in(['balcao', 'whatsapp', 'telefone', 'site', 'parceiro'])],
            'status' => ['required', Rule::in(['solicitada', 'em_analise', 'confirmada', 'retirada', 'concluida', 'cancelada'])],
            'observacoes' => ['nullable', 'string', 'max:5000'],
        ]);
        unset($dados['id']);

        if ($dados['motocicleta_id'] ?? null) {
            $conflito = $this->scope(Reserva::query(), $request->user())
                ->where('motocicleta_id', $dados['motocicleta_id'])
                ->whereNotIn('status', ['concluida', 'cancelada'])
                ->when($reserva->exists, fn (Builder $query) => $query->whereKeyNot($reserva->id))
                ->where('retirada_em', '<', $dados['devolucao_em'])
                ->where('devolucao_em', '>', $dados['retirada_em'])
                ->exists();

            if ($conflito) {
                return back()->withInput()->withErrors([
                    'motocicleta_id' => 'Esta motocicleta já possui uma reserva no período informado.',
                ]);
            }
        }
        ...
        $reserva->fill($dados)->save();

        return $this->voltar('reservas', $reserva->wasRecentlyCreated ? 'Reserva criada com sucesso.' : 'Reserva atualizada com sucesso.');
    }
    ...
    public function salvarLoja(Request $request): RedirectResponse
    {
        ...
        return redirect()
            ->route('rental.index', ['page' => 'lojas', 'edit' => $loja->id])
    ...
    public function salvarCliente(Request $request): RedirectResponse
    {
        ...
        foreach (['foto_cliente', 'foto_documento', 'comprovante_residencia'] as $campo) {
            if ($request->hasFile($campo)) {
                $dados[$campo] = $request->file($campo)->store('clientes', config('filesystems.private', 'local'));
            } else {
                unset($dados[$campo]);
            }
        }
    ...
    {
        $this->autorizar($request->user(), 'clientes', 'visualizar');
        $cliente = $this->scope(Cliente::query(), $request->user())->findOrFail($cliente->id);
        abort_unless(in_array($campo, ['foto_cliente', 'foto_documento', 'comprovante_residencia'], true), 404);

        $path = (string) $cliente->{$campo};
        abort_if($path === '', 404, 'Documento não encontrado.');

        $privateDisk = config('filesystems.private', 'local');
        if (Storage::disk($privateDisk)->exists($path)) {
            return Storage::disk($privateDisk)->download($path);
        }

        // Compatibilidade temporária para documentos gravados antes da versão comercial.
        abort_unless(Storage::disk('public')->exists($path), 404, 'Documento não encontrado.');
    ...
    public function salvarCrmCliente(Request $request, Cliente $cliente): RedirectResponse
    {
        ...
        return redirect()->route('rental.index', ['page' => 'crm', 'cliente' => $cliente->id])->with('success', 'Etapa do cliente atualizada.');
    }
    ...
    public function salvarCrmNota(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('rental.index', ['page' => 'crm', 'cliente' => $cliente->id])->with('success', 'Nota adicionada ao CRM.');
    }
    ...
    public function salvarCrmTarefa(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('rental.index', ['page' => 'crm', 'cliente' => $cliente->id])->with('success', $mensagem);
    }
    ...
    public function concluirCrmTarefa(Request $request, CrmTarefa $tarefa): RedirectResponse
    {
        ...
        return redirect()->route('rental.index', ['page' => 'crm', 'cliente' => $tarefa->cliente_id])->with('success', 'Tarefa concluida.');
    }
    ...
    public function responderPortalAtendimento(Request $request, PortalAtendimento $atendimento): RedirectResponse|JsonResponse
    {
        ...
        if (($atendimento->canal ?? 'portal') === 'telegram') {
            ...
            if (! ($envioTelegram['ok'] ?? false)) {
                ...
                return redirect()
                    ->route('rental.index', ['page' => 'crm', 'cliente' => $atendimento->cliente_id])
    ...
    public function acaoPortalAtendimento(Request $request, PortalAtendimento $atendimento): RedirectResponse|JsonResponse
    {
        ...
        if ($dados['acao'] === 'reabrir') {
            ...
            if ($outroAtivo) {
                ...
                return redirect()
                    ->route('rental.index', ['page' => 'crm', 'atendimento' => $outroAtivo->id])
        ...
        return redirect()
            ->route('rental.index', ['page' => 'crm', 'atendimento' => $atendimento->id])
    ...
    public function conciliarPix(Request $request): RedirectResponse
    {
        ...
        $resultado = $this->pixGateway->conciliarPendentes(
            (int) config('rental.pix.conciliacao_limite', 50)
        );
    ...
    {
        $this->autorizar($request->user(), 'telegram', 'editar');
        $resultado = $this->telegram->configurarWebhooks(route('rental.webhook-telegram'));
    ...
    public function salvarPagBank(Request $request): RedirectResponse
    {
        ...
        $dados['webhook_url'] = $dados['webhook_url'] ?: route('rental.webhook-pagbank');
        ...
        if ($request->input('page') === 'bancos') {
            return redirect()->route('rental.index', ['page' => 'bancos', 'banco' => 'pagbank', 'loja_id' => $lojaId])->with('success', $mensagem);
        }
    ...
    public function salvarAsaas(Request $request): RedirectResponse
    {
        ...
        $dados['webhook_url'] = $dados['webhook_url'] ?: route('rental.webhook-asaas');
        $dados['webhook_token'] = $dados['webhook_token'] ?: RentalSupport::webhookToken('asaas');
        ...
        if ($request->input('page') === 'bancos') {
            return redirect()->route('rental.index', ['page' => 'bancos', 'banco' => 'asaas', 'loja_id' => $lojaId])->with('success', $mensagem);
        }
    ...
    public function salvarSicoob(Request $request): RedirectResponse
    {
        ...
        $dados['webhook_token'] = $dados['webhook_token'] ?: RentalSupport::webhookToken('sicoob');
        $dados['webhook_url'] = $dados['webhook_url'] ?: route('rental.webhook-sicoob', ['token' => $dados['webhook_token']]);
        ...
        if ($request->input('page') === 'bancos') {
            return redirect()->route('rental.index', ['page' => 'bancos', 'banco' => 'sicoob', 'loja_id' => $lojaId])->with('success', $mensagem);
        }
    ...
    public function salvarItau(Request $request): RedirectResponse
    {
        ...
        $dados['webhook_token'] = $dados['webhook_token'] ?: RentalSupport::webhookToken('itau');
        $dados['webhook_url'] = $dados['webhook_url'] ?: route('rental.webhook-itau', ['token' => $dados['webhook_token']]);
        unset($dados['acao'], $dados['page'], $dados['loja_id']);
        ...
        if ($request->input('page') === 'bancos') {
            return redirect()->route('rental.index', ['page' => 'bancos', 'banco' => 'itau', 'loja_id' => $lojaId])->with('success', $mensagem);
        }
    ...
    public function salvarGatewayPix(Request $request): RedirectResponse
    {
        ...
        if ($page === 'bancos') {
            return redirect()->route('rental.index', ['page' => 'bancos', 'banco' => $dados['gateway'], 'loja_id' => $lojaId])->with('success', 'Gateway PIX principal atualizado.');
        }
    ...
    public function salvarUsuarioPerfil(Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($perfil, $dados, $codigo): void {
            ...
            foreach ($dados['perms'] ?? [] as $modulo => $acoes) {
                foreach (array_keys($acoes) as $acao) {
                    if (isset(RentalSupport::MODULOS[$modulo], RentalSupport::ACOES[$acao])) {
        ...
        return redirect()->route('rental.index', ['page' => 'usuarios', 'perfil_edit' => $perfil->id])
    ...
    private function dashboard(User $user): array
    {
        ...
        $cobrancas = $this->scope(Cobranca::query(), $user);
        $contratos = $this->scope(Contrato::query(), $user);
        $reservas = $this->scope(Reserva::query(), $user);
        $ordensServico = $this->scope(OrdemServico::query(), $user);
        ...
        return [
            'reservasEmPreparacao' => (clone $reservas)
                ->whereIn('status', ['solicitada', 'em_analise', 'confirmada'])
                ->count(),
            'retiradasHoje' => (clone $contratos)->whereDate('data_inicio', today())->count(),
            'devolucoesHoje' => (clone $contratos)
                ->whereDate('data_fim', today())
                ->whereNotIn('status', ['encerrado'])
                ->count(),
            'manutencoesAtencao' => (clone $ordensServico)
                ->whereNotIn('status', ['concluida', 'cancelada'])
                ->whereNotNull('previsto_em')
                ->whereDate('previsto_em', '<=', today())
                ->count(),
    ...
    private function formatarAtendimentoInbox(PortalAtendimento $atendimento): array
    {
        ...
        return [
            ...
            'atendente_nome' => $atendimento->atendente?->nome,
            'url' => route('rental.index', ['page' => 'crm', 'atendimento' => $atendimento->id]),
        ];
    }
    ...
    private function crmTimeline(Cliente $cliente)
    {
        ...
        Cobranca::query()
            ...
            ->each(fn (Cobranca $cobranca) => $items->push([
                ...
                'tipo' => 'Cobranca',
                'titulo' => 'Cobranca #'.$cobranca->id.' - '.RentalSupport::moeda($cobranca->valor_principal),
        ...
        DB::table('pagamentos as p')
            ...
            ->each(fn ($pagamento) => $items->push([
                ...
                'texto' => RentalSupport::moeda($pagamento->valor).' via '.$pagamento->forma,
                'status' => 'ok',
            ]));
    ...
    {
        return RentalSupport::limitarPorLoja($query, $user, $coluna);
    }
    ...
    {
        return redirect()->route('rental.index', ['page' => $page])->with('success', $mensagem);
    }
}
