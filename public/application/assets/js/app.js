function rentalApplyTheme(theme){
  ...
  try{ localStorage.setItem('rental-theme', next); }catch(e){}
...
(function(){
  const saved = (()=>{ try{return localStorage.getItem('rental-theme') || 'light';}catch(e){return 'light';} })();
  rentalApplyTheme(saved);
  document.addEventListener('DOMContentLoaded', ()=>{
    rentalApplyTheme(document.documentElement.dataset.theme || saved);
    document.querySelectorAll('[data-theme-toggle]').forEach(button=>{
      button.addEventListener('click', ()=>{
        rentalApplyTheme(document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark');
      });
    });
  });
})();

function rentalBars(id, labels, values){
...
(function(){
  ...
  function chatAppend(thread, remetente, mensagem, hora, id, criadoEm, clientToken, pending, remetenteNome){
    ...
    const storeName=document.body?.dataset.storeName || 'a locadora';
    const nome=remetente==='cliente' ? 'Você' : (remetente==='humano' ? (remetenteNome || 'Equipe '+storeName) : (remetenteNome || 'Lau'));
    ...
    const avatar=remetente==='cliente' ? '' : '<span class="chat-mini-avatar"><img src="/application/assets/img/atendente-lauro.png" alt="Lau"></span>';
  ...
  function crmNomeRemetente(remetente, nome){
    ...
    if(tipo==='humano') return 'Atendente · '+(nome || 'Equipe '+(document.body?.dataset.storeName || 'da locadora'));
...
/* =========================================================
   APPLICATION V6.3 - GRÁFICOS EXECUTIVOS PREMIUM
   Donuts e barras com legendas alinhadas, fontes maiores e layout limpo.
   ========================================================= */
function rentalFormatValue(value, prefix){
...
function rentalDonutPremium(id, items, prefix){
  const el=document.getElementById(id); if(!el) return;
  ...
  const main = prefix === 'R$' ? rentalFormatValue(rawTotal, 'R$') : rentalFormatValue(rawTotal);
  el.innerHTML = `
    ...
    <div class="donut-premium-legend">
      ${items.map(it=>{
        const val = rentalFormatValue(it.value, prefix);
...
function rentalMiniBarsPremium(id, labels, values, prefix){
  const el=document.getElementById(id); if(!el) return;
  ...
  el.innerHTML = `<div class="mini-bars-list">${labels.map((label,i)=>{
    ...
    return `<div class="mini-bar-row"><div class="mini-bar-top"><span>${label}</span><b>${rentalFormatValue(val,prefix)}</b></div><div class="mini-bar-track"><i style="width:${width}%"></i></div></div>`;
  }).join('')}</div>`;
}
